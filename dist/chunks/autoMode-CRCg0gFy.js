import{Br as e,Dr as t,Ga as n,Mr as r,Pr as i,oo as a}from"./paths-BdgeTjAU.js";import{E as o,O as s,U as c,z as l}from"./debug-A-rhqP-J.js";import{$_ as u,Q_ as d,X_ as f,Z_ as p,ev as m}from"./loadAgentsDir-DDEzZ0Zj.js";import{n as h,t as g}from"./poorMode-CQw0mkCE.js";c(),i(),d(),a(),g(),u(),o();function _(e){process.stdout.write(s(e,null,2)+`
`)}function v(){_(p())}function y(){let e=n(),t=p();_({allow:e?.allow?.length?e.allow:t.allow,soft_deny:e?.soft_deny?.length?e.soft_deny:t.soft_deny,environment:e?.environment?.length?e.environment:t.environment})}var b=`You are an expert reviewer of auto mode classifier rules for Claude Code.
...
Be concise and constructive. Only comment on rules that could be improved. If all rules look good, say so.`;async function x(i){let a=n();if(!((a?.allow?.length??0)>0||(a?.soft_deny?.length??0)>0||(a?.environment?.length??0)>0)){process.stdout.write(`No custom auto mode rules found.

Add rules to your settings file under autoMode.{allow, soft_deny, environment}.
Run \`claude auto-mode defaults\` to see the default rules for reference.
`);return}let o=i.model?e(i.model):h()?r():t(),s=p(),c=f(),u=S(`allow`,a?.allow??[],s.allow)+S(`soft_deny`,a?.soft_deny??[],s.soft_deny)+S(`environment`,a?.environment??[],s.environment);process.stdout.write(`Analyzing your auto mode rules…
