import{Ir as e,Pr as t,Ur as n,_o as r,kr as i,no as a}from"./paths-DJg_jbfO.js";import{E as o,O as s,U as c,z as l}from"./debug-B_528_ux.js";import{av as u,iv as d,nv as f,rv as p,tv as m}from"./loadAgentsDir-Bmfyrm8o.js";import{n as h,t as g}from"./poorMode-BSxJsuPG.js";c(),e(),p(),r(),g(),d(),o();function _(e){process.stdout.write(s(e,null,2)+`
`)}function v(){_(f())}function y(){let e=a(),t=f();_({allow:e?.allow?.length?e.allow:t.allow,soft_deny:e?.soft_deny?.length?e.soft_deny:t.soft_deny,environment:e?.environment?.length?e.environment:t.environment})}var b=`You are an expert reviewer of auto mode classifier rules for Claude Code.
...
Be concise and constructive. Only comment on rules that could be improved. If all rules look good, say so.`;async function x(e){let r=a();if(!((r?.allow?.length??0)>0||(r?.soft_deny?.length??0)>0||(r?.environment?.length??0)>0)){process.stdout.write(`No custom auto mode rules found.

Add rules to your settings file under autoMode.{allow, soft_deny, environment}.
Run \`claude auto-mode defaults\` to see the default rules for reference.
`);return}let o=e.model?n(e.model):h()?t():i(),s=f(),c=m(),d=S(`allow`,r?.allow??[],s.allow)+S(`soft_deny`,r?.soft_deny??[],s.soft_deny)+S(`environment`,r?.environment??[],s.environment);process.stdout.write(`Analyzing your auto mode rules…
