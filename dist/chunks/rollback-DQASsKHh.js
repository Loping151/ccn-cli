async function e(e,t){if(t?.list){console.log(`Recent versions:`),console.log(`  (version listing requires access to the release registry)`),console.log("  Use `ccn update --list` for available versions.");return}if(t?.safe){if(console.log(`Safe rollback: would install the server-pinned safe version.`),t.dryRun){console.log(`  (dry run — no changes made)`);return}console.log(`  Safe version pinning requires access to the release API.`),console.log(`  Contact oncall for the current safe version.`);return}if(!e){console.error(`Usage: ccn rollback [target]
