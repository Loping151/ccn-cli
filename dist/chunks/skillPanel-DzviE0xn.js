import{n as e,o as t}from"./chunk-DR8-3Aex.js";import{ii as n,ri as r}from"./paths-CIl-ozdy.js";import{Rn as i,b as a,f as o,t as s,x as c,xt as l}from"./src-Kfqxxbsd.js";import{t as u}from"./jsx-runtime-BLYyAvvR.js";import{OS as d,jS as f}from"./loadAgentsDir-atKeDaX0.js";import{r as p,t as m}from"./featureCheck-Bo2G9w5C.js";function h(){let e=p();return r()===`zh`?`# Skill Learning (自动学习)
...
`}async function g(){let{readObservations:e,loadInstincts:t,resolveProjectContext:n}=await import(`./skillLearning-pUVCqceA.js`),r=n(process.cwd()),[i,a]=await Promise.all([e({project:r}),t({project:r})]);return[`Skill Learning status for ${r.projectName} (${r.projectId})`,`Observations: ${i.length}`,`Instincts: ${a.length}`,``,`Skill Learning: ${p()?`enabled`:`disabled`}`].join(`
`)}async function _(){let e=[];p()?e.push(`Skill Learning: already enabled`):(process.env.SKILL_LEARNING_ENABLED=`1`,e.push(`Skill Learning: enabled (SKILL_LEARNING_ENABLED=1)`));try{let{initSkillLearning:t}=await import(`./runtimeObserver-BFXKAs-0.js`);t(),e.push(`Runtime observer: initialized`)}catch{e.push(`Runtime observer: init skipped (not available)`)}return e.join(`
...
`)}function y({onDone:e}){f(`skill-panel`);let[t,n]=(0,x.useState)(0),r=(0,x.useMemo)(()=>[{label:`Status`,description:`Show skill learning status for current project`,run:g},{label:`Start`,description:`Enable skill learning for this session`,run:_},{label:`Stop`,description:`Disable skill learning for this session`,run:v},{label:`About`,description:`Detailed description of skill learning features`,run:()=>Promise.resolve(h())}],[]),i=()=>{let n=r[t];n&&n.run().then(t=>{e(t,{display:`system`})})};return l((e,t)=>{if(t.upArrow){n(e=>Math.max(0,e-1));return}if(t.downArrow){n(e=>Math.min(r.length-1,e+1));return}t.return&&i()}),(0,S.jsx)(o,{title:`Skill Learning`,subtitle:`${r.length} actions`,onCancel:()=>e(`Skill panel dismissed`,{display:`system`}),color:`background`,hideInputGuide:!0,children:(0,S.jsxs)(c,{flexDirection:`column`,children:[r.map((e,n)=>(0,S.jsxs)(c,{flexDirection:`row`,children:[(0,S.jsx)(a,{children:`${n===t?`›`:` `} ${e.label}`.padEnd(C)}),(0,S.jsx)(a,{dimColor:!0,children:e.description})]},e.label)),(0,S.jsx)(c,{marginTop:1,children:(0,S.jsx)(a,{dimColor:!0,children:`↑/↓ select · Enter run · Esc close`})})]})})}async function b(e,t,n){let r=n?.trim()??``;if(r===`start`)return e(await _(),{display:`system`}),null;if(r===`stop`)return e(await v(),{display:`system`}),null;if(r===`about`)return e(h(),{display:`system`}),null;if(r===`status`)return e(await g(),{display:`system`}),null;if(r){let{call:t}=await import(`./skill-learning-ZB1wu-OW.js`),n=await t(r,{});return n&&typeof n==`object`&&`value`in n&&e(n.value,{display:`system`}),null}return(0,S.jsx)(y,{onDone:e})}var x,S,C;e((()=>{x=t(i(),1),s(),d(),m(),n(),S=u(),C=28}))();export{b as call};